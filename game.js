...
const CASES_BLOQUEES = ['montagne']; // seule la montagne bloque — l'eau donne -1 force comme le piège
...
function activerDefense() {
  ...
  unite.bonusForce = (unite.bonusForce || 0) + 1; // +1 force permanent (comme bonus-attaque)
...
function lancerCombat() {
  ...
  animerDe('de-combat-att', 'chiffre-combat-att', (deAtt) => {
    animerDe('de-combat-def', 'chiffre-combat-def', (deDef) => {
      ...
      const scoreDef = deDef + FORCE[uniteDef.type] + (uniteDef.bonusForce || 0); // bonusForce contient déjà le +1 défense
...
function finDeTourJoueur() {
  // Retirer l'effet visuel "défense" des jetons du joueur actif
  // Note : le bonusForce reste permanent (il ne se remet pas à zéro)
  for (let l = 0; l < 8; l++) {
    for (let c = 0; c < 8; c++) {
